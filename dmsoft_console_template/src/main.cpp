...
void main_thread()
{
	...
	// 綁定窗口
	cout << "綁定窗口...";
	dm_ret = dm->BindWindow(hwnd, _T(BIND_DISPLAY), _T(BIND_MOUSE), _T(BIND_KEYPAD), BIND_MODE);
	if (dm_ret != 1)
	{
		cout << "失敗(" << dm_ret << ")" << endl;
		return;
...
int main()
{
	...
	// 初始化MFC
	cout << "初始化MFC...";
	if (AfxWinInit(::GetModuleHandle(NULL), NULL, ::GetCommandLine(), 0) != TRUE)
	{
		cout << "失敗" << endl;
		return -1;
	}
    ...
    if (g_dm->IsFileExist(_T("license.txt")) == 0)
	{
		...
		cout << "失敗，創建\"license.txt\"文件，如需註冊大漠插件，請填入註冊碼" << endl;
	}
	else
	{
